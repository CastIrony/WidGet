...
open class Node: Equatable, Hashable {
    ...
    private static let EMPTY_NODES = [Node]()
    ...
    private func addSiblingHtml(_ index: Int, _ html: String) throws {
        ...
        let nodes: [Node] = try Parser.parseFragment(html, context, getBaseUri())
    ...
    open func wrap(_ html: String) throws -> Node? {
        ...
        let context: Element? = parent() as? Element
        var wrapChildren: [Node] = try Parser.parseFragment(html, context, getBaseUri())
...
//        }
    ...
    open func nextSibling() -> Node? {
        guard let siblings: [Node] = parentNode?.childNodes else {
            return nil
        }
    ...
    open func traverse(_ nodeVisitor: NodeVisitor) throws -> Node {
        let traversor = NodeTraversor(nodeVisitor)
