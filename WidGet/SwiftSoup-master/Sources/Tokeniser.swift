...
final class Tokeniser {
    ...
    private let charsBuilder = StringBuilder(1024) // buffers characters to output as one token, if more than one emit per read
    let dataBuffer = StringBuilder(1024) // buffers data looking for </script>

    var tagPending = Token.Tag() // tag we are building up
    let startPending = Token.StartTag()
    let endPending = Token.EndTag()
    let charPending = Token.Char()
    let doctypePending = Token.Doctype() // doctype building up
    let commentPending = Token.Comment() // comment building up
