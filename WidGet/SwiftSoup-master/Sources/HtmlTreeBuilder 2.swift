...
class HtmlTreeBuilder: TreeBuilder {
    ...
    private var _state = HtmlTreeBuilderState.Initial // the current state
    private var _originalState = HtmlTreeBuilderState.Initial // original / marked state
    ...
    private var formattingElements = [Element?]() // active (open) formatting elements
    private var pendingTableCharacters = [String]() // chars in table to be shifted out
    private var emptyEnd = Token.EndTag() // reused empty end tag
    ...
    override public func defaultSettings() -> ParseSettings {
    ...
    func insertEmpty(_ startTag: Token.StartTag) throws -> Element {
        ...
        try Validate.notNull(obj: startTag._attributes)
        let el = Element(tag, baseUri, startTag._attributes)
        try insertNode(el)
        if startTag.isSelfClosing() {
            if tag.isKnownTag() {
    ...
    func insertForm(_ startTag: Token.StartTag, _ onStack: Bool) throws -> FormElement {
        let tag: Tag = try Tag.valueOf(startTag.name(), settings)
        try Validate.notNull(obj: startTag._attributes)
        let el = FormElement(tag, baseUri, startTag._attributes)
        setFormElement(el)
        try insertNode(el)
        if onStack {
            stack.append(el)
    ...
    func insert(_ commentToken: Token.Comment) throws {
        let comment = Comment(commentToken.getData(), baseUri)
    ...
    private func isElementInQueue(_ queue: [Element?], _ element: Element?) -> Bool {
    ...
    private func replaceInQueue(_ queue: [Element], _ out: Element, _ input: Element) throws -> [Element] {
        var queue = queue
    ...
    func newPendingTableCharacters() {
        pendingTableCharacters = [String]()
    }

    func getPendingTableCharacters() -> [String] {
        return pendingTableCharacters
    }

    func setPendingTableCharacters(_ pendingTableCharacters: [String]) {
